...
jvm.classParser = (function(){
  ...
  function read_constant_pool(reader, constant_pool_count){
    ...
    var pool_size = constant_pool_count - 1;
    var constant_pool = [undefined];

    for(var i = 0; i < pool_size; i++){
      var tag = reader.readUint8();
      var const_info = constInfoFactories[tag]();
      constant_pool.push(const_info);
    }
  ...
  return {
    parse: function(arrayBuffer){
      ...
      klass.constant_pool = read_constant_pool(reader,
                                               klass.constant_pool_count);
      return klass;
